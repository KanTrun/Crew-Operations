...
export default function HaoPhiPage() {
  ...
  const theoMatHang = useMemo<GomMatHang[]>(() => {
    const filteredGhiChu = ghiChu.filter((g) => {
      if (!matchSearch([safeText(g.ghi_chu, ""), safeText(g.mat_hang, ""), safeText(g.nguyen_nhan, "")].join(" "), search)) return false;
      if (!matchTime(g.created_at, timeF)) return false;
      return true;
    });
    const m = new Map<string, GomMatHang & { nn: Map<string, number> }>();
    for (const g of filteredGhiChu) {
  ...
  const filteredCumTheoThu = useMemo(
    () => cumTheoThu.filter((it) => matchSearch([safeText(it.cau, ""), safeText(it.thu, "")].join(" "), search)),
    [cumTheoThu, search],
  );

  const filterActive = search.length > 0 || timeF !== "all";
    ...
    <main className="min-h-screen p-4 md:p-8 pb-32 relative" ref={containerRef}>
      ...
      <div className="ops-animate-in mb-8">
        <ListToolbar
          ...
          searchPlaceholder="Tìm ghi chú, mặt hàng, nguyên nhân…"
          time={timeF}
          onTimeChange={(v) => setTimeF(v as TimeFilter)}
          timeOptions={TIME_FILTER_OPTIONS}
          shown={filteredGhiChu.length}
          total={ghiChu.length}
      ...
      <div className="ops-animate-in mb-12">
        ...
        {!loading && cumTheoThu.length > 0 && filteredCumTheoThu.length === 0 ? <FilteredEmpty onClear={clearFilters} /> : null}
        {!loading && filteredCumTheoThu.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredCumTheoThu.map((it, i) => (
              <div 
      ...
      {!loading && filteredGhiChu.length > 0 ? (
        <div className="ops-animate-in mb-12">
          <h2 className="text-2xl font-black uppercase mb-6 text-[var(--nq-fg)] flex items-center gap-4">
            Lần hao gần đây
            <span className="text-sm bg-[var(--nq-copper)] text-[#0e0c0a] px-3 py-1 rounded-full">{Math.min(filteredGhiChu.length, 8)}</span>
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredGhiChu.slice(0, 8).map((g, i) => (
              <div 
