...
export function FloatingChatHead() {
  ...
  return (
    <div className="fixed bottom-20 md:bottom-6 right-4 md:right-6 z-40 flex flex-col items-end">
      {notification ? (
        <div role="status" className="mb-2 max-w-[min(360px,calc(100vw-2rem))] rounded-lg border border-[var(--nq-accent)] bg-[var(--nq-bg-elevated)] px-3 py-2 text-xs text-[var(--nq-fg)] shadow-xl">
          {notification}
        </div>
      ) : null}
      ...
      {isOpen && (
        <div className="mb-3 flex h-[min(500px,calc(100vh-7rem))] w-[calc(100vw-2rem)] max-w-[380px] flex-col overflow-hidden rounded-2xl border border-[var(--nq-dim)] bg-[var(--nq-bg-elevated)] shadow-2xl animate-fade-in">
          {/* Header */}
          <div className="p-3 bg-[var(--nq-accent)] text-[var(--nq-accent-ink)] flex items-center justify-between">
            ...
            {messages.slice(-20).map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                  <div className={`w-fit max-w-[82%] rounded-xl border p-2 ${isMine ? "border-[var(--nq-accent)] bg-[var(--nq-accent)] text-[var(--nq-accent-ink)]" : "border-[var(--nq-dim)] bg-[var(--nq-card)] text-[var(--nq-fg)]"}`}>
                    <div className={`mb-0.5 flex gap-3 text-2xs ${isMine ? "justify-end text-[var(--nq-accent-ink)]/70" : "justify-between text-[var(--nq-muted)]"}`}>
                      {!isMine ? <span className="font-bold text-[var(--nq-accent)]">{msg.sender_name || msg.sender_id}</span> : null}
          ...
          <form onSubmit={handleQuickSend} className="p-2 border-t border-[var(--nq-dim)] bg-[var(--nq-card)] flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-1.5 rounded-xl bg-[var(--nq-bg)] border border-[var(--nq-dim)] text-xs text-[var(--nq-fg)] outline-none focus:border-[var(--nq-accent)]"
            />
            <button
              type="submit"
              disabled={!input.trim()}
              className="px-3 py-1.5 rounded-xl bg-[var(--nq-accent)] text-[var(--nq-accent-ink)] font-bold text-xs hover:opacity-90 disabled:opacity-40 transition"
            >
              ➤
            </button>
          </form>
