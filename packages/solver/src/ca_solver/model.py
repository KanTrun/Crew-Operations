...
@dataclass
class LichInput:
    """Minimal schedule payload for hard-constraint validation."""
    ...
    # Callers must pass legal params from config/tham-so-lao-dong.yaml
...
def solve_hard_only(data: LichInput) -> SolveResult:
    ...
    return SolveResult(ok=len(violations) == 0, violations=violations)
