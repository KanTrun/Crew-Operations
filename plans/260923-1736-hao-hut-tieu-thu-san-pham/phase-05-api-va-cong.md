...
- [x] `pytest apps/api -q` xanh, gồm `test_capability_coverage.py`.
- [x] `GET /api/v1/hao-hut` với KV rỗng trả `tong_dong: 0`, **không** raise.
- [x] Dòng `sua_tuoi` khớp số với `kiem_ke` (test đọc thẳng seed để đối chiếu).
- [x] `POST /api/v1/hao-hut` xuất hiện trong `/api/v1/vet` (audit có ghi).
- [x] `_tong_ket_ngay` đếm đúng số bản ghi trong ngày (test với `luc`).
- [x] `GET` và `POST` cũ ở `/api/v1/waste` vẫn xanh (test cũ không sửa).
