...
- [ ] Cổng `test_architecture.py` xanh (không import chéo).
- [ ] Hỏi "hao hụt tuần này do đâu" qua API trả `intent=ANALYZE_LOSS`.
- [ ] Khi KV rỗng, tool trả `co_du_lieu: False` — test khẳng định điều này.
- [ ] `explanation` luôn nêu nguồn (`kiem_ke`, `menu_mon.bom`, `waste_notes`).
