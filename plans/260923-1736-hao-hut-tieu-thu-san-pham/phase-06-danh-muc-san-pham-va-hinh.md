...
- [x] `python scripts/seed_danh_muc.py` rồi `GET /api/v1/menu/quan-tri` trả ≥ 45 món.
- [x] Chạy script hai lần: số món **không** tăng; món quán tự thêm còn nguyên.
- [x] `python scripts/sinh_anh_mon.py` sinh ảnh cho **mọi** món; chạy lại ra cùng byte.
- [x] `GET /api/v1/menu/{id}/anh` trả 200 ảnh hợp lệ cho mọi món, **khi tắt mạng**.
- [x] Có nước đóng chai trong danh mục mặc định của `persist.py`.
- [x] `ruff check scripts` sạch.
