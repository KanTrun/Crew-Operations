...
- [ ] `ruff check apps/api/src packages scripts` sạch.
- [ ] `pytest` xanh toàn bộ.
- [ ] `cd apps/web; npx tsc --noEmit` sạch.
- [ ] `npx next build` thành công, route `/hao-phi` có trong output.
- [ ] e2e xanh (hoặc nêu rõ spec nào đỏ và vì sao, kèm bằng chứng).
- [ ] `make contracts` lần hai không đổi file nào.
- [ ] `git log` có commit cho từng phase, đúng conventional commit + scope hợp lệ.
- [ ] `docs/adr/019-*.md` tồn tại và nêu rõ lý do chọn sinh ảnh tại máy.
