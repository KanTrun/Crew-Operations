...
- [ ] `npm run lint` (tsc) sạch.
- [ ] `/hao-phi` hiển thị bảng hao hụt theo nguyên liệu với số thật từ API.
- [ ] Dòng `thieu_du_lieu` nói rõ thiếu vế nào (lý thuyết hay thực tế).
- [ ] Mọi món ở `/menu` hiện ảnh (không rơi về chữ cái đầu).
- [ ] `/tieu-thu` → `/hao-phi` và `/menu` → `/hao-phi` bấm được.
- [ ] e2e `/hao-phi` cũ (flows.spec.ts) vẫn xanh; e2e mới xanh.
- [ ] Không có `[object Object]` hay mã trạng thái thô trên UI.
