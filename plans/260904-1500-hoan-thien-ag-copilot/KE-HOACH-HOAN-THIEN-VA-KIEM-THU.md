...
- [x] PR10 còn lại — TKB confirm, consent đổi ca, bàn giao: 3 mutating intent `PROPOSE_TKB_CONFIRM` (mọi role, ownership gate: non-manager chỉ xác nhận TKB của mình — mirror route web `chi_gan_tkb_cua_minh`, validate khoảng bận `T2..T7,CN + HH:MM` giống `_clean_khoang_api`), `PROPOSE_SWAP_CONSENT` (mọi role, chỉ người tham gia {a,b,c} hoặc quan_ly/chu_quan được đồng ý, append nv_id vào `dong_y`, chuyển `trang_thai="dong_y"` khi cả a,b đồng ý — mirror route `/cho-doi-ca/{id}/dong-y`), `PROPOSE_HANDOVER` (mọi role, ghi bàn giao SBAR qua `extract_handover` API layer + `validate_num` VF số, append `handover_history` keep last 50 — mirror route POST `/handover`); 3 enum mới + matrix cấp cho cả 3 role (giữ `nhan_vien ⊆ quan_ly`); 3 `_cap` mới trong CAPABILITY_REGISTRY (`PROPOSE_TKB_CONFIRM`, `PROPOSE_SWAP_CONSENT`, `PROPOSE_HANDOVER`); 3 tool mới trong tool_registry với fail-closed (`khoang_rong`, `chi_gan_tkb_cua_minh`, `swap_not_found`, `khong_phai_nguoi_tham_gia`, `missing_handover_text`) + snapshot live; executor ghi cùng KV key/schema route web (`tkb_nv`/`swap`/`handover_history`) trong transaction nội bộ, retry-safe + correction schema riêng; `copilot_agent` truyền `user_id`/`user_role` xuống tool (cũ vẫn nhận qua `**kwargs`); `EXCLUDED_ROUTES` cập nhật cho `/api/v1/tkb/confirm` và `/api/v1/cho-doi-ca/{id}/dong-y` + `/api/v1/doi-ca/{id}/xac-nhan` (đã phủ qua chat). Validation: `104 passed` (5 suites — test_chat_api 7, test_copilot_api 61, test_mail_quality_gate 4, test_no_network 1, test_ag_copilot 31), Ruff pass, mypy strict pass 112 files (10 pre-existing errors từ `persist.py`/`worker.py` không liên quan — đã verify với `git stash` baseline).
