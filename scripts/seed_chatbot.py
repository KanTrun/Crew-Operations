...
def seed_chatbot_data(db_path: Path = None) -> None:
    ...
    sample_questions_hours = json.dumps(
        ["Mở mấy giờ?", "Các bạn đóng cửa lúc mấy giờ?", "Mở hôm nay không?", "Còn mở thêm không?"],
        ensure_ascii=False,
    )
    ...
    intents = [
        ...
        (
            "menu_query",
            "Hỏi về menu",
            "Hỏi về menu, giá cà phê, đồ ăn",
            sample_questions_menu,
            0,
            1,
        ),
        (
            "reservation",
            "Đặt bàn/tổ chức sự kiện",
            "Yêu cầu đặt bàn hoặc tổ chức sự kiện",
            sample_questions_reservation,
            1,
            0,
        ),
        ...
        (
            "feedback",
            "Góp ý/khiếu nại",
            "Phản hồi, góp ý, hoặc khiếu nại",
            sample_questions_feedback,
            1,
            0,
        ),
        (
        ...
        (
            "other",
            "Khác",
            "Các câu hỏi khác không rõ ý định",
    ...
    for intent_id, display_name, description, samples, requires_approval, auto_enabled in intents:
        cursor.execute(
            ...
            (id, display_name, description, sample_questions,
             requires_approval, auto_response_enabled, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
    ...
    rules = [
        ...
        (
            ...
            "payment",
            None,
            "💳 Chúng tôi chấp nhận: Tiền mặt, Chuyển khoản QR, Ví điện tử (Momo, ZaloPay). "
    ...
    for rule_id, intent, condition, response, threshold, enabled, created_by in rules:
        cursor.execute(
            ...
            (id, intent, condition, response_template,
             confidence_threshold, enabled, created_by_nv_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
    ...
    kb_entries = [
        (
            ...
            "hours",
            "giờ mở cửa",
            "Quán mở 6h sáng - 22h tối hàng ngày",
            json.dumps(["config/tham-so-lao-dong.yaml"], ensure_ascii=False),
            1.0,
            None,
        ),
        (
            ...
            json.dumps(["data/menu_mon"], ensure_ascii=False),
            0.95,
            "menu_mon",
        ),
        (
            ...
            json.dumps(["SOP step 5"], ensure_ascii=False),
            0.9,
            None,
        ),
        (
            ...
            "order",
            "giao hàng",
            "Hỗ trợ giao hàng trong nội thành. Phí giao từ 15k. Thời gian giao 30-45 phút.",
            json.dumps(["SOP step 7"], ensure_ascii=False),
            0.85,
            None,
        ),
        (
            ...
            json.dumps(["SOP step 3"], ensure_ascii=False),
            0.8,
            None,
        ),
    ]
    ...
    for kb_id, category, key_phrase, content, sources, confidence, dynamic_table in kb_entries:
        cursor.execute(
            ...
            (id, category, key_phrase, content,
             sources, confidence, dynamic_from_table, last_updated)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
