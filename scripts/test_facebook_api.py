...
class FacebookAPITester:
    ...
    def _make_request(
        ...
        try:
            print(f"\n📤 {method} {endpoint}")
            response = requests.request(method=method, url=url, params=params, timeout=10)
    ...
    def test_page_feeds(self, page_id: str, limit: int = 5) -> dict[str, Any]:
        ...
        data = self._make_request(f"{page_id}/feed", params={"fields": fields, "limit": limit})
