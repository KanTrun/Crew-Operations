...
class FacebookPagePoster:
    ...
    def get_post_info(self, post_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ Post info retrieved")
                print(f"   Created: {data.get('created_time', 'N/A')}")
                print(f"   Likes: {data.get('likes', {}).get('summary', {}).get('total_count', 0)}")
                print(f"   Comments: {data.get('comments', {}).get('summary', {}).get('total_count', 0)}")
...
def test_suite():
    ...
    text_content = f"🤖 Bài test tự động từ Nhịp Quán Bot\n\nThời gian: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n✅ Test post thành công!"
    ...
    link_result = poster.post_link(
        url="https://github.com/nhipquan",
        message="GitHub Repository - Nhịp Quán Project"
    )
